...
NodePtr expr(size_t min_binding, lexeme_stream &stream,
             const std::shared_ptr<relaxed_parser_listener_t> &listener);

void disallow_pure_atoms_in_parens(
    lexeme_stream &stream,
    const std::shared_ptr<relaxed_parser_listener_t> &listener) {
    if (is_atom(stream.peek(1).type()) &&
        stream.peek(2).type() == lexeme_type::RParen) {
        listener->didFindPureAtomInParens(stream.current(), stream.peek(1),
                                          stream.peek(2));
...
NodePtr
handle_parens(lexeme_stream &stream,
              const std::shared_ptr<relaxed_parser_listener_t> &listener) {
...
NodePtr parse_nud(lexeme_stream &stream,
                  const std::shared_ptr<relaxed_parser_listener_t> &listener,
                  lexeme &l) {
    NodePtr lhs;
...
std::shared_ptr<syntax_tree> relaxed_parser::parse(
