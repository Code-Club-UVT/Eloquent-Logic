...
public:
  ...
  void
  traverse_children(const std::function<void(const NodeObsPtr &)> &cb) const;
  ...
  void
  condense(const std::function<void(NodeObsPtr, NodeObsPtr)> &on_merge = {});
  ...
  void set_parent(const NodeObsPtr &parent) { this->parent = parent; }
