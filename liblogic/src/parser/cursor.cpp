...
size_t Cursor::get_arity() const { return c_node->arity(); }

bool Cursor::has_child(size_t idx) const {
    return idx < c_node->num_children();
}

bool Cursor::tree_is_empty() const noexcept { return this->tree->empty(); }
...
void Cursor::replace_child_with_placeholders(lexeme l) {
    ...
    NodePtr root =
        Node::make_node(lexeme::make(lexeme_type::Unknown, "", 0, 0));
...
void Cursor::spawn_new_child_node() {
    ...
    listener->spawnedNewChildNode(c_node,
                                  c_node->childAt(c_node->num_children() - 1));
}
