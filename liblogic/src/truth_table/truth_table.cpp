...
void truth_table::emplace_or_update(table_row &cache, CppCommon::UUID id,
                                    bool value) {
...
void truth_table::evaluate_impl(
    const interpretation &i, table_row &cache,
            ...
            if (is_nary_operator(node->getLexeme().type())) {
                node->traverse_children([&](NodeObsPtr cnode) {
                    if (result == std::nullopt)
                        ...
                        result =
                            combine(node->getType(), node->getLexeme(),
                                    result.value(), get_value(cache, cnode));
                });
                emplace_or_update(cache, node->getUUID(), result.value());
                listener->didComputeSubexpression(node->getUUID(),
                                                  result.value());
            } else {
                node->traverse_children([&](NodeObsPtr cnode) {
                    result = get_value(cache, cnode);
...
std::vector<table_row> truth_table::evaluate_all(
    const std::shared_ptr<truth_table_listener_t> &listener) {
    ...
    for (size_t i = 0; i < (static_cast<size_t>(1) << num_vars);
         ++i) { // TODO: this is horribly inefficient
        interpretation it;
...
table_row
truth_table::evaluate(const interpretation &i,
                      const std::shared_ptr<truth_table_listener_t> &listener) {
    table_row tr;
    listener->didStart();
...
truth_table
truth_table::make_from_tree(const std::shared_ptr<syntax_tree> &tree) {
    truth_table t(tree);
    // add atoms
    tree->walk([&t](auto node) {
        using enum lexeme_type;
        if (node->getLexeme().type() == Atom &&
            !t.m_interpretation.contains(node->getLexeme().token())) {
