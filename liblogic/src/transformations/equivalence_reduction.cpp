...
namespace eloquent::logic {
    ...
    void EquivalenceReduction::replace(const NodeObsPtr target, const std::shared_ptr<node_transformation_listener_t>& listener) {
        ...
        listener->didAdoptNode(target->childAt(1)->childAt(0),target->childAt(1)->childAt(0)->lastChild());
        target->childAt(1)->childAt(1)->adopt(std::move(rs));
        listener->didAdoptNode(target->childAt(1)->childAt(1),target->childAt(1)->childAt(1)->lastChild());
