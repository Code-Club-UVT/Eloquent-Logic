...
bool Inverter::match(
    ...
    bool result =
        subtree->getType() == NodeType::NotOp &&
        (subtree->childAt(0)->getLexeme().type() == lexeme_type::Tautology ||
         subtree->childAt(0)->getLexeme().type() == lexeme_type::Contradiction);
    if (result)
        listener->didMatchInverter(subtree);
    return result;
}
...
void Inverter::replace(
    ...
    if (child_type == lexeme_type::Tautology) {
        target->set_lexeme(lexeme::make(
            lexeme_type::Contradiction, symbols::SYMB_CONTRADICTION,
            child_lexeme.start(), child_lexeme.end()));
        listener->didTransformNodeWithLexeme(target, target->getLexeme());
    } else {
        target->set_lexeme(
            lexeme::make(lexeme_type::Tautology, symbols::SYMB_TAUTOLOGY,
                         child_lexeme.start(), child_lexeme.end()));
        listener->didTransformNodeWithLexeme(target, target->getLexeme());
    }
