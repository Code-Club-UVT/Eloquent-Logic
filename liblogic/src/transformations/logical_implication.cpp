...
namespace eloquent {
namespace logic {
bool logical_implication::match(
    NodeObsPtr subtree,
    const std::shared_ptr<node_transformation_listener_t> &listener) {
    bool result = subtree->getType() == NodeType::LEquiOp;
    if (result)
        listener->didMatchLogicalImplication(subtree);
    return result;
}

void logical_implication::replace(
    NodeObsPtr target,
    const std::shared_ptr<node_transformation_listener_t> &listener) {
    ...
    target->set_lexeme(lexeme::make(lexeme_type::AndOp, symbols::SYMB_AND,
                                    c_lexeme.start(), c_lexeme.end()));
    listener->didTransformNodeWithLexeme(target, target->getLexeme());

    target->condense([&](NodeObsPtr parent, NodeObsPtr merged) {
