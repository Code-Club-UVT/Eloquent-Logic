...
NeutralElements::Signal NeutralElements::get_case(const NodeObsPtr &node) {
    ...
    node->traverse_children([&](auto n) {
        if (n->getLexeme().type() == lexeme_type::Tautology)
            count_tautology++;
        else if (n->getLexeme().type() == lexeme_type::Contradiction)
            count_contradiction++;
    });

    // case 1 (high priority): a contradiction in a conjunction subtree
    if (node->getType() == NodeType::AndOp && count_contradiction) {
        return Signal::CONJUNCTION_CONTRADICTION;
    }

    // case 2 (high priority): a tautology in a disjunction subtree
    if (node->getType() == NodeType::OrOp && count_tautology) {
        return Signal::DISJUNCTION_TAUTOLOGY;
    }

    // case 3: a tautology in an n-ary conjunction subtree
    if (node->getType() == NodeType::AndOp && count_tautology) {
        return Signal::CONJUNCTION_TAUTOLOGY;
    }
    ...
    if (node->getType() == NodeType::OrOp && count_contradiction) {
        return Signal::DISJUNCTION_CONTRADICTION;
    }
    // base case
    return Signal::CLEAR;
}

// The following 2 functions could be made more efficient if, instead of the
// node, we pass them the signal itself, but I feel like this could cause
// complications in the long run.

bool NeutralElements::match(
...
void NeutralElements::replace(
    const NodeObsPtr target,
    const std::shared_ptr<node_transformation_listener_t> &listener) {

    // It is a good idea to free unused nodes from memory.
    // Although freeing the nodes is optional, since we are using smart
    // pointers, it is a good improvement for larger trees and longer runtimes.
    ...
    // I am unsure whether it may interfere with the wanted order of operations,
    // though.
    ...
    while (flag != Signal::CLEAR) {
        ...
        if (flag == Signal::CONJUNCTION_CONTRADICTION) {
            ...
            target->set_lexeme(lexeme::make(lexeme_type::Contradiction,
                                            symbols::SYMB_CONTRADICTION, 0, 0));
            ...
            target->set_lexeme(lexeme::make(lexeme_type::Tautology,
                                            symbols::SYMB_TAUTOLOGY, 0, 0));
            listener->didTransformNodeWithLexeme(target, target->getLexeme());
            target->clear_children();
            listener->didDiscardAllChildren(target);
            ...
            do {
                ...
                target->traverse_children([&](auto n) {
                    if ((target->getType() == NodeType::AndOp &&
                         n->getLexeme().type() == lexeme_type::Tautology) ||
                        (target->getType() == NodeType::OrOp &&
                         n->getLexeme().type() == lexeme_type::Contradiction))
