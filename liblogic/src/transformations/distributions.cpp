...
bool and_distribution::match(
    ...
    subtree->traverse_children([&](const NodeObsPtr &child) {
        if (child->getType() == NodeType::OrOp)
            found = true;
    });
    if (found)
        listener->didMatchAndDistribution(subtree);
    return found;
}

void and_distribution::replace(
    NodeObsPtr target,
    const std::shared_ptr<node_transformation_listener_t> &listener) {
    ...
    size_t or_idx = 0;
    while (others[or_idx]->getType() != NodeType::OrOp)
        ++or_idx;
    ...
    for (size_t i = 0; i < disjuncts.size(); ++i) {
        ...
        listener->didSpawnNewSubtree(
            target->childAt(target->num_children() - 1));

        const NodeObsPtr conjunction =
            target->childAt(target->num_children() - 1);

        const bool last = (i + 1 == disjuncts.size());
        for (auto &other : others) {
            conjunction->adopt(last ? std::move(other)
                                    : Node::duplicate_node(other.get()));
            listener->didSpawnNewSubtree(
                conjunction->childAt(conjunction->num_children() - 1));
            listener->didAdoptNode(
                conjunction,
                conjunction->childAt(conjunction->num_children() - 1));
        }
        ...
        listener->didSpawnNewSubtree(
            conjunction->childAt(conjunction->num_children() - 1));
        listener->didAdoptNode(
            conjunction, conjunction->childAt(conjunction->num_children() - 1));
    }

    listener->didDistributeAndOverOr(target);
}

bool and_distribution::should_apply() { return false; }
...
bool or_distribution::match(
    ...
    bool found = false;
    subtree->traverse_children([&](const NodeObsPtr &child) {
        if (child->getType() == NodeType::AndOp)
            found = true;
    });
    if (found)
        listener->didMatchOrDistribution(subtree);
    return found;
}

void or_distribution::replace(
    NodeObsPtr target,
    const std::shared_ptr<node_transformation_listener_t> &listener) {
    std::vector<NodePtr> others;
    while (target->num_children() > 0) {
    ...
    size_t and_idx = 0;
    while (others[and_idx]->getType() != NodeType::AndOp)
        ++and_idx;
    ...
    const lexeme l = target->getLexeme();
    target->set_lexeme(lexeme::make(lexeme_type::AndOp, symbols::SYMB_AND,
                                    l.start(), l.end()));
    listener->didTransformNodeWithLexeme(target, target->getLexeme());

    for (size_t i = 0; i < conjuncts.size(); ++i) {
        target->spawn_new_child(
            lexeme::make(lexeme_type::OrOp, symbols::SYMB_OR, 0, 0));
        listener->didSpawnNewSubtree(
            target->childAt(target->num_children() - 1));
        listener->didAdoptNode(target,
                               target->childAt(target->num_children() - 1));

        const NodeObsPtr disjunction =
            target->childAt(target->num_children() - 1);

        const bool last = (i + 1 == conjuncts.size());
        for (auto &other : others) {
            disjunction->adopt(last ? std::move(other)
                                    : Node::duplicate_node(other.get()));
            listener->didSpawnNewSubtree(
                disjunction->childAt(disjunction->num_children() - 1));
            listener->didAdoptNode(
                disjunction,
                disjunction->childAt(disjunction->num_children() - 1));
        }
        ...
        listener->didSpawnNewSubtree(
            disjunction->childAt(disjunction->num_children() - 1));
        listener->didAdoptNode(
            disjunction, disjunction->childAt(disjunction->num_children() - 1));
    }

    listener->didDistributeOrOverAnd(target);
}

bool or_distribution::should_apply() { return false; }
} // namespace eloquent::logic
