/**
 * Pentru acest program vom folosi strategia de alegere 'la întâmplare', numită
 * și RAND
 */
#include "dpll_rand.h"
#include <chrono>
...
class ProblemContext {
  ...
  public:
    [[nodiscard]] ProblemContext(const HeuristicsDB_DPLL &db,
                                 const ClauseSet &cs)
        : db(db), cs(cs) {}
    [[nodiscard]] HeuristicsDB_DPLL &get_db() { return db; }
...
bool one_literal_clause_rule(ClauseSet &cs, std::set<Literal> &single_literals,
                             SatState &result, HeuristicsDB_DPLL &db,
                             const std::shared_ptr<sat_listener> &listener) {
...
bool single_polarity_rule(ClauseSet &cs, HeuristicsDB_DPLL &db,
...
bool davis_putnam(ProblemContext &pc, SatState &result,
    ...
    if (one_literal_clause_rule(pc.get_cs(), single_literals, result,
                                pc.get_db(), listener))
        return true;
    if (single_polarity_rule(pc.get_cs(), pc.get_db(), single_polarity_literals,
                             result, listener))
        return true;
    return false;
}
...
Literal pick_literal(ProblemContext &ctx,
    ...
    std::uniform_int_distribution<size_t> distrib(
        0, ctx.get_db().get_record().size() - 1);
...
SatState
davis_putnam_logemann_loveland(const HeuristicsDB_DPLL &db, const ClauseSet &cs,
                               const std::shared_ptr<sat_listener> &listener) {
    ...
    while (!context_stack.empty()) {
        ...
        if (c_solved) {
            // this context's clause set contains an empty clause (result ==
            // UNSAT): a dead branch, drop it and move on to the next one on the
            // stack.
