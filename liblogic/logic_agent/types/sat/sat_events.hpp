...
//   - didBuildHeuristicsDatabase(const ClauseSet&)                    -> clause_set_event_t (mapper_events.hpp)
//   - didFindUnitLiteral(Literal) / didDeriveEmptyClauseFromUnitPropagation(Literal) /
//     didFindPureLiteral(Literal) / didChooseRandomLiteral(Literal)   -> literal_event_t
//   - didSatisfyClauseByUnitLiteral(Clause, Literal) /
//     didEliminateClauseByPureLiteral(Clause, Literal)                -> clause_literal_event_t
//   - didReduceClauseByUnitLiteral(Clause original, Clause reduced, Literal) -> clause_reduction_event_t
//   - didSelectResolutionCandidates(Clause, Clause)                   -> clause_pair_event_t
//   - didComputeResolvent(Clause, Clause, Literal pivot, Clause resolvent) -> resolvent_event_t
//   - didSkipDuplicateResolvent(Clause) / didAddResolvent(Clause)     -> clause_event_t
//   - didExceedClauseThreshold(size_t)                                -> clause_count_event_t
//   - didChooseBranchingLiteral(Literal, int frequency)               -> branching_literal_event_t
//   - didEnterBranch(Literal, bool) / didPushBranchContext(Literal, bool) -> branch_event_t
//   - didDeriveEmptyClauseFromResolution() / didReachResolutionSaturation() /
//     didPopBranchContext() / didFindSatisfyingAssignment() / didDetectConflict() /
//     didBacktrack() / didConcludeSat() / didConcludeUnsat() / didConcludeUnknown() -> no payload
namespace logic_agent::types::sat
{
    ...
    NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(branching_literal_event_t, literal, frequency);
