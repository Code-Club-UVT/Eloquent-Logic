...
//   - didTransformNodeWithLexeme(Node* node, lexeme) -> transform_lexeme_event_t
//   - didTransferNode(Node* source, size_t index, Node* destination) -> transfer_node_event_t
...
namespace logic_agent::types::transformations
{
    ...
    NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(transfer_node_event_t, source, index, destination)
