...
parse_formula(const std::string &formula, bool strict, event_sink &sink) {
    ...
    if (strict) {
        auto parser_listener =
            std::make_shared<streaming_strict_parser_listener>(sink);
        return eloquent::logic::strict_parser::parse(lexemes, parser_listener);
    }

    auto parser_listener =
        std::make_shared<streaming_relaxed_parser_listener>(sink);
...
parse_formula_from_params(const nlohmann::json &params, event_sink &sink) {
    if (!params.is_object() || !params.contains("formula") ||
        !params.at("formula").is_string()) {
        throw rpc_exception(error_code::invalid_params,
                            R"(params is missing a string "formula")");
    }
