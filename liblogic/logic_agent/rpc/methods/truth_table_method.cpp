...
{
    nlohmann::json handle_truth_table(const nlohmann::json& params, event_sink& sink)
    {
        ...
        auto variable_value = [&](std::size_t variable_index, std::size_t row_index)
        {
            return static_cast<bool>(row_index & (static_cast<std::size_t>(1) << variable_index));
        };
        ...
        auto root_value = [&](std::size_t row_index) -> bool
        {
            ...
            switch (root_lexeme.type())
            {
                ...
                {
                    const auto it = std::find(variables.begin(), variables.end(), root_lexeme.token());
                    return variable_value(static_cast<std::size_t>(std::distance(variables.begin(), it)), row_index);
                }
                case lexeme_type::Tautology: return true;
                case lexeme_type::Contradiction: return false;
                default: return rows[row_index].at(root_uuid);
            }
        };
