...
nlohmann::json handle_sat(const nlohmann::json &params, event_sink &sink) {
    ...
    auto transform_listener =
        std::make_shared<streaming_transformation_listener>(sink);
    runner.to_cnf(tree, transform_listener);

    auto mapper_listener = std::make_shared<streaming_mapper_listener>(sink);
    auto mapping_result =
        eloquent::logic::mapping_generator::gen_map(tree, mapper_listener);
    ...
    if (algorithm == "dp") {
        ...
        throw rpc_exception(
            ...
            R"("algorithm" must be "dp", "dpll_mcl", "dpll_rand", or "resolution")");
    }
