...
- **[`openrpc.json`](./openrpc.json)** — an [OpenRPC](https://spec.open-rpc.org/) 1.3.2
  document for the 6 request/response methods a client actually calls: `parse`,
  `transform`, `sat`, `logical_consequence`, `truth_table`, `shutdown`. Method
  params/results and the AST (`Node`) shape they share live here. `shutdown` takes no
  params and, unlike the other five, ends the server's request loop right after its
  Response is sent.
- **[`feedback-events.schema.json`](./feedback-events.schema.json)** — a plain
  ...
  notifications. It catalogues all distinct `"<category>/<callbackName>"`
  notifications logic_agent can stream (lexer/parser/transform/sat/mapper/
  logical_consequence/truth_table callbacks) and their payload shapes — `shutdown`
  streams none of its own, it's a bare control command. `$defs.AnyFeedbackNotification`
  validates any one parsed line of the stream without knowing its method name up front.
