...
namespace
{
    ...
    {
        NodePtr n = Node::make_node(lexeme::make(lexeme_type::NotOp, symbols::SYMB_NOT, 0, 0));
    ...
    ClauseSet map_to_clauses(const std::shared_ptr<syntax_tree>& tree)
    {
        ...
        {
            root->traverse_children([&](const NodeObsPtr& clause_node) { clauses.emplace(clause_from(clause_node)); });
        }
        else
        {
    ...
    std::shared_ptr<syntax_tree> three_clause_sat_formula()
    {
        return tree_from(
            op(lexeme_type::AndOp, symbols::SYMB_AND,
                ...
                op(lexeme_type::AndOp, symbols::SYMB_AND,
                    op(lexeme_type::OrOp, symbols::SYMB_OR, negate(atom("A")), atom("C")),
                    op(lexeme_type::OrOp, symbols::SYMB_OR, negate(atom("B")), negate(atom("C"))))));
    }
} // namespace

TEST(TransformationRunnerToCnfTest, TerminatesAndFlattensAThreeClauseConjunction)
{
...
TEST(TransformationRunnerToCnfTest, ResolutionAgreesOnAnUnsatFourClauseConjunction)
{
    // (A \vee B) \wedge (\neg A \vee C) \wedge \neg B \wedge \neg C — unsat,
...
TEST(TransformationRunnerToDnfTest, TerminatesAndFlattensAThreeTermDisjunction)
{
