...
class recording_listener : public truth_table_listener_t {
  public:
    ...
    void didComputeSubexpression(const CppCommon::UUID &id,
                                 bool value) override {
        computed.emplace_back(id, value);
    }

    [[nodiscard]] std::optional<bool>
    valueFor(const CppCommon::UUID &id) const {
...
TEST_F(TruthTableTest, CollectsEachDistinctAtomOnceAsAVariable) {
    // P & Q | P
    auto tree =
        build({Atom("P"), Sym(lexeme_type::AndOp, symbols::SYMB_AND), Atom("Q"),
...
TEST_F(TruthTableTest, ExcludesTautologyAndContradictionFromVariables) {
    // top & P & bot
    auto tree =
        build({Sym(lexeme_type::Tautology, symbols::SYMB_TAUTOLOGY),
               Sym(lexeme_type::AndOp, symbols::SYMB_AND), Atom("P"),
...
TEST_F(TruthTableTest, EvaluatesOr) {
    auto tree =
        build({Atom("P"), Sym(lexeme_type::OrOp, symbols::SYMB_OR), Atom("Q")});
    auto tt = truth_table::make_from_tree(tree);
    auto root_id = tree->rootRef()->getUUID();
...
TEST_F(TruthTableTest, EvaluatesImplies) {
    auto tree =
        build({Atom("P"), Sym(lexeme_type::ImpliesOp, symbols::SYMB_IMPL),
               Atom("Q")});
    auto tt = truth_table::make_from_tree(tree);
    auto root_id = tree->rootRef()->getUUID();
...
TEST_F(TruthTableTest, EvaluatesDoubleNegation) {
    auto tree = build({Sym(lexeme_type::NotOp, symbols::SYMB_NOT),
...
TEST_F(TruthTableTest, EvaluatesCompositeExpressionRespectingPrecedence) {
    ...
    auto eval = [&](bool p, bool q, bool r) {
        return tt.evaluate({{"P", p}, {"Q", q}, {"R", r}}, null_listener)
            .at(root_id);
    };
...
TEST_F(TruthTableTest, ListenerObservesStartFinishAndSetVariables) {
    ...
    EXPECT_THAT(listener->set_variables,
...
TEST_F(TruthTableTest, EvaluateAllMatchesTheExpectedAssignmentForEveryRow) {
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
        ...
        for (size_t j = 0; j < vars.size(); ++j)
            assignment[vars[j]] =
                static_cast<bool>(i & (static_cast<size_t>(1) << j));

        bool expected =
            !assignment.at("P") || (assignment.at("Q") && assignment.at("R"));
...
TEST_F(TruthTableTest, ThrowsUnsupportedOperatorErrorForModelsOperator) {
    ...
    auto tree = build(
        {Atom("P"), Sym(lexeme_type::LEquiOp, symbols::SYMB_LEQUI), Atom("Q")});
    auto tt = truth_table::make_from_tree(tree);

    EXPECT_THROW(tt.evaluate({{"P", true}, {"Q", true}}, null_listener),
                 unsupported_operator_error);
}

TEST_F(TruthTableTest,
       UnsupportedOperatorErrorMessageMentionsTheOperatorToken) {
    auto tree = build(
        {Atom("P"), Sym(lexeme_type::LEquiOp, symbols::SYMB_LEQUI), Atom("Q")});
    auto tt = truth_table::make_from_tree(tree);
    ...
    try {
        ...
        EXPECT_NE(std::string(e.what()).find(symbols::SYMB_LEQUI),
                  std::string::npos);
    }
}
